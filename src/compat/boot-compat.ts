...
let readFile: (p: string) => string = function (): string {
...
const loader = createModLoader({
    engine: engine,
    readFile: function (p: string): string { return readFile(p); },
    tag: TAG
});

// 宿主注入存储读取能力（同步 readFileSync 语义：路径 → 文本；读不到返回 ''）
g.__mixinSetModSource = function (opts: { readFile: (p: string) => string }): void {
    if (opts && typeof opts.readFile === 'function') readFile = opts.readFile;
};
// 装载 <root>/mods.json 列表（两种格式兼容）并注册 patches；entry 进队列
g.__mixinLoadMods = function (root: string): { registered: number; total: number } {
    return loader.loadMods(root);
};
// 装载单个 mod 目录（宿主自管列表时用）
g.__mixinLoadOne = function (root: string, id: string): void { loader.loadOne(root, id); };
// entry 队列：游戏/应用启动后由宿主在合适的挂点调用（v1 语义）
g.__mixinRunEntries = function (): void { loader.runEntries(); };
// 注册窗口关闭通知（目标代码开始装载时调用；未装载成功则告警）
g.__mixinWindowClosed = function (): void { loader.windowClosed(); };

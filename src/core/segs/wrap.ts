...
function topStatements(node: any): any[] {
    if (node.type === 'Program') return node.body || [];
    if (node.body && node.body.type === 'BlockStatement') return node.body.body || [];
    return [];
}

function unwrapParens(n: any): any {
    while (n && n.type === 'ParenthesizedExpression') n = n.expression;
    return n;
}

export function resolveWrap(node: any, seg: PathSeg, src: string): ResolveResult {
    const kind = seg.wrap as string;
    const stmts = topStatements(node);
    const candidates: any[] = [];
    const pushFactory = (call: any): void => {
    ...
    for (let i = 0; i < stmts.length; i++) {
        ...
        if (st.type === 'ExpressionStatement') {
            ...
            if (kind === 'iife' && expr.type === 'CallExpression') pushFactory(expr);
                ...
                && expr.argument.type === 'CallExpression') pushFactory(expr.argument); // !function(){}()
            else if (kind === 'cjs' && expr.type === 'AssignmentExpression'
                && dottedName(expr.left) === 'module.exports' && expr.right.type === 'CallExpression') pushFactory(expr.right);
                ...
                if (c && isFnNode(c) && c.params && c.params.length >= 1) {
                    ...
                    for (let a = expr.arguments.length - 1; a >= 0; a--) {
                        ...
                        if (arg && isFnNode(arg) && hasBlockBody(arg)) { candidates.push(arg); break; }
                    }
                }
            }

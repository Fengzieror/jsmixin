...
export interface MixinEngine {
    ...
    stats(): { mods: number; version: string };
 ...
 */
function applyReplaces(mx: MixinEntry, source: string, filename: string, log: MixinLog): string {
    let out = source;
...
function byPriority(a: ModDescription, b: ModDescription): number {
    const pa = a.priority == null ? 1000 : (a.priority as number);
    const pb = b.priority == null ? 1000 : (b.priority as number);
    return pa - pb;
}

export function createMixinEngine(options?: EngineOptions): MixinEngine {
    const log = (options && options.log) || defaultLog;
    if (options && options.acorn) setAcorn(options.acorn);
    const mods: ModDescription[] = []; // 已注册的 mod 描述列表

    function transformSerial(filename: string, source: string): string {
    ...
    function matchingMixins(filename: string): { mod: ModDescription; mx: MixinEntry }[] {
        const base = baseName(filename);
        const ordered = mods.slice().sort(byPriority);
        const out: { mod: ModDescription; mx: MixinEntry }[] = [];
        for (let mi = 0; mi < ordered.length; mi++) {
    ...
    return {
        register: function (mod: ModDescription): void {
        ...
        transformFile: function (filename: string, source: string): string {
            try {
        ...
        reset: function (): void { mods.length = 0; },

 ...
 */
import { isFnNode, hasBlockBody, calleeName, walkAll, describeFn, logable, directChildFns, fnFeatures, matchAnchor } from './features.js';
...
/* ---------- acorn 注入 ---------- */
let _acorn = null;
export function setAcorn(a) { _acorn = a; }
function getAcorn() {
    if (_acorn)
        return _acorn;
...
/* ---------- 路径段解析 ---------- */
export function resolvePath(ast, path, src) {
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        let r;
        if (seg.module != null)
            r = resolveModule(node, seg, src);
        else if (seg.name != null)
            r = resolveName(node, seg, src);
        else if (seg.call != null)
            r = resolveCall(node, seg, src);
        else if (seg.fn != null)
            r = resolveFnIndex(node, seg, src);
            ...
            r = resolveClass(node, seg, src);
        else if (seg.wrap != null)
            r = resolveWrap(node, seg, src);
        else if (seg.anchor)
            r = resolveAnchor(node, seg, src);
        else
 ...
 */
export function applyAstPatches(filename, source, patches, stats) {
    const acorn = getAcorn();
    if (!acorn) {

/* 由 jsmixin v2 构建生成（源: src/compat/boot-compat.ts），勿手改。 */
(() => {
    ...
    walkAll(node, function(n) {
      ...
      for (let i = 0; i < props.length; i++) {
        ...
        for (let j = 0; j < el.properties.length; j++) {
          const pp2 = el.properties[j];
          if (pp2.type !== "Property" && pp2.type !== "ObjectProperty") continue;
          const pn = pp2.key && (pp2.key.name || pp2.key.value);
          if (pn === "key") keyVal = pp2.value && (pp2.value.type === "Literal" ? pp2.value.value : pp2.value.name);
          else if (pn === "value" || pn === "get" || pn === "set") valNode = pp2.value;
        }
  ...
  function applyOp(fn, patch, src, edits) {
    ...
    if (op === "inject") {
      const at2 = patch.at || "head";
      validateBodyCode(code, "inject-" + at2);
      if (at2 === "head") edits.push({ start: fn.body.start + 1, end: fn.body.start + 1, text: "\n" + code });
      else if (at2 === "tail") {
      ...
      } else throw new Error("inject at \u4EC5\u652F\u6301 head/tail\uFF0C\u6536\u5230 " + at2);
  ...
  function byPriority(a, b) {
    const pa = a.priority == null ? 1e3 : a.priority;
    const pb = b.priority == null ? 1e3 : b.priority;
    return pa - pb;
  }
  function createMixinEngine(options) {
    const log = options && options.log || defaultLog;
    if (options && options.acorn) setAcorn(options.acorn);
    const mods = [];
    function transformSerial(filename, source) {
      ...
      for (let mi = 0; mi < ordered.length; mi++) {
        ...
        for (let i = 0; i < mixins.length; i++) {
          ...
          let out = source;
          if (mx.replaces) out = applyReplaces(mx, out, filename, log);
    ...
    function transform(filename, source) {
      ...
      if (matched.length <= 1) return transformSerial(filename, source);
      let batch = [];
    ...
    return {
      register: function(mod) {
        ...
        const mixins = mod.mixins || [];
        for (let i = 0; i < mixins.length; i++) mixins[i]._modid = mod.modid || "?";
        mods.push(mod);
        log('registered mod "' + (mod.modid || "?") + '" v' + (mod.version || "?") + " with " + mixins.length + " mixin(s)");
      },
      ...
      sourceHash,
      stats: function() {
        return { mods: mods.length, version: VERSION2 };
      },
      version: VERSION2
    };
  }
  ...
  function installAstGlobals(g2 = globalThis) {
    g2.__mixinAst = {
      version: VERSION,
      applyAstPatches,
  ...
  function installTransformerGlobals(g2 = globalThis) {
    const engine2 = createMixinEngine({ acorn: g2.acorn });
    function log(msg) {
      try {
        console.log("[mixin] " + msg);
      } catch (e) {
    ...
    g2.__mixin = {
      version: VERSION2,
      register: function(mod) {
        engine2.register(mod);
      },
      stats: function() {
        return { mods: engine2.stats().mods, evalHooked: !!g2.__mixinEvalHooked };
      },
      // 供补丁作者/测试计算期望哈希：__mixin.sourceHash(source) → 'fnv1a32:xxxx:len:nnn'
      sourceHash: engine2.sourceHash
    };
    g2.__mixinTransform = function(filename, source) {
      try {
        return engine2.transformFile(filename || "(unknown)", source);
      } catch (e) {
        log("transform error on " + filename + ": " + e);
        return source;
      }
    ...
    function installEvalHook() {
      if (g2.__mixinEvalHooked) return;
      const origEval = g2.eval;
      if (typeof origEval !== "function") {
        log("WARN: eval \u4E0D\u53EF\u7528\uFF0Ceval \u94A9\u5B50\u672A\u5B89\u88C5");
        return;
      }
      g2.eval = function(src) {
        if (typeof src === "string" && src.length > 64) {
          const m = /\/\/@ sourceURL=(.+?)\s*$/.exec(src);
          try {
            src = g2.__mixinTransform(m ? m[1] : "(inline-eval)", src);
          } catch (e) {
      ...
      g2.__mixinEvalHooked = true;
      log("eval hook installed (v" + VERSION2 + ")");
    }
    installEvalHook();
    return engine2;
  }
  ...
  function createModLoader(options) {
    const engine2 = options.engine;
    const readFile2 = options.readFile;
    ...
    function loadOne(root, id) {
      const dir = root + "/" + id;
      const mfTxt = readFile2(dir + "/mixins.json");
      if (!mfTxt) {
      ...
      for (let m = 0; m < mixinFiles.length; m++) {
        const code = readFile2(dir + "/" + mixinFiles[m]);
        if (!code) {
      ...
      if (manifest.entry) {
        const entryTxt = readFile2(dir + "/" + manifest.entry);
        if (entryTxt) {
          entries.push(/* @__PURE__ */ function(code, mid) {
            return function() {
    ...
    function loadMods(root) {
      loadedOk = true;
      lastRoot = root;
      const listTxt = readFile2(root + "/mods.json");
      let list = null;
      if (listTxt) {
        try {
      ...
      return { registered, total: list.length };
    }
    return {
      loadMods,
      ...
      entries,
      reload: function() {
        if (lastRoot) loadMods(lastRoot);
      }
    };
  }

  // src/compat/boot-compat.ts
  setAcorn(acorn_exports);
  installAstGlobals();
  var engine = installTransformerGlobals();
  var g = globalThis;
  var TAG = "[mixin-boot]";
  ...
  g.__mixinSetModSource = function(opts) {
    if (opts && typeof opts.readFile === "function") readFile = opts.readFile;
  };
  g.__mixinLoadMods = function(root) {
    return loader.loadMods(root);
  };
  g.__mixinLoadOne = function(root, id) {
    loader.loadOne(root, id);
  };
  g.__mixinRunEntries = function() {
    loader.runEntries();
  };
  g.__mixinWindowClosed = function() {
    loader.windowClosed();
  };
})();

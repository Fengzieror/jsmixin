...
exports.resolveMethodAt = resolveMethodAt;
/*
...
function classMethodCandidates(clsNode, method) {
    ...
    for (let i = 0; i < body.length; i++) {
        const md = body[i];
        if (!md || md.type !== 'MethodDefinition' || md.computed)
            continue;
        const kv = md.key && (md.key.type === 'Identifier' ? md.key.name : (md.key.type === 'Literal' ? md.key.value : null));
        if (kv === method && (0, features_js_1.isFnNode)(md.value) && (0, features_js_1.hasBlockBody)(md.value))
            candidates.push(md.value);
    }
    return candidates;
}
    ...
    (0, features_js_1.walkAll)(node, function (n) {
        ...
        for (let i = 0; i < props.length; i++) {
            ...
            for (let j = 0; j < el.properties.length; j++) {
                ...
                if (pp.type !== 'Property' && pp.type !== 'ObjectProperty')
                    continue;
                const pn = pp.key && (pp.key.name || pp.key.value);
                if (pn === 'key')
                    keyVal = pp.value && (pp.value.type === 'Literal' ? pp.value.value : pp.value.name);
                else if (pn === 'value' || pn === 'get' || pn === 'set')
                    valNode = pp.value;
            }
            if (keyVal === method && (0, features_js_1.isFnNode)(valNode) && (0, features_js_1.hasBlockBody)(valNode))
                candidates.push(valNode);
        }
    });
    return candidates;
}
function resolveMethod(node, seg, src, prevAsName) {
    ...
    if ((0, features_js_1.isClassNode)(node)) {
        return (0, pick_js_1.pick)(classMethodCandidates(node, method), seg, src, 'method ' + method + ' of class');
    }
    const clsName = seg.cls || prevAsName || null;
    return (0, pick_js_1.pick)(methodTableCandidates(node, clsName, method), seg, src, 'method ' + method + (clsName ? ' of ' + clsName : ''));
}
